...
install_deps(){
  ...
  libs="python-pyside libgstreamer-plugins-base0.10-dev libgstreamer0.10-cil-dev libgstreamer0.10-dev python-gst0.10-dev rygel-gst-launch gstreamer0.10-plugins-good python-sphinx gstreamer0.10-tools python-gtk2 python-gtk2-dev gstreamer0.10-alsa"
...
get_sphinx(){
  version="0.8"
  cd /home/$(whoami)
  wget http://downloads.sourceforge.net/cmusphinx/sphinxbase-$version.tar.gz
...
get_pocketsphinx(){
  version="0.8"
  cd /home/$(whoami)
  wget http://downloads.sourceforge.net/cmusphinx/pocketsphinx-$version.tar.gz
...
get_blather(){
  ...
  git clone https://github.com/ivan-leontiev/blather.git
}
